...
class Repository:
    def __init__(self, dir_path):
        ...
        self.students_path = os.path.join(dir_path, 'HW12_students.txt')
        self.instructors_path = os.path.join(dir_path, 'HW12_instructors.txt')
        self.grades_path = os.path.join(dir_path, 'HW12_grades.txt')
        self.major_path = os.path.join(dir_path, 'HW12_majors.txt')
    ...
    def instructors_Table_Flask_web(self, DB_File):
        ...
        def instructors():
            try:
                ...
                return f"Error: Unable to open database file {DB_File}"
            else:
                query = "select CWID,Name,Dept,Course,count(StudentCWID) as num_of_students \
                        from HW11_instructors join HW11_grades on CWID = InstructorCWID \
                        group by CWID,Name,Dept,Course \
                ...
                data = [{'cwid': CWID, 'name': Name, 'department': Dept, 'course': Course, 'students': num_of_students}
                        for CWID, Name, Dept, Course, num_of_students in db.execute(query)]
